...
export function generateExamRulesDynamically(total, mcq, clientDifficultyRules) {
  const flatRules = [];
...
function validateExamStructure(exam) {
  for (const q of exam.questions) {
    if (q.type === "TF") {
      if (q.questionText.trim().endsWith("?")) return { valid: false, reason: `TF must be a statement: ${q.questionText}` };
      if (q.options?.length > 0) return { valid: false, reason: "TF cannot contain options" };
      if (!["True", "False"].includes(q.correctAnswer)) return { valid: false, reason: "TF answer must be True or False" };
    }
    if (q.type === "MCQ") {
      if (!q.options || q.options.length !== 4) return { valid: false, reason: "MCQ must contain exactly 4 options" };
      if (!q.options.includes(q.correctAnswer)) return { valid: false, reason: "MCQ correctAnswer must exist in options" };
    }
  }
  return { valid: true };
}
...
async function reviewerAgent(state) {
  ...
  if (!validation.valid) return { reviewVerdict: "FAILED", reviewFeedback: validation.reason };
  ...
  if (result.toUpperCase().startsWith("FAILED")) return { reviewVerdict: "FAILED", reviewFeedback: result };
  return { reviewVerdict: "PASSED" };
}

function routeAfterReview(state) {
  if (state.reviewVerdict === "FAILED") { console.log("🔄 Failed -> Regenerating"); return "generator"; }
...
const workflow = new StateGraph(graphState)
  ...
  .addConditionalEdges("reviewer", routeAfterReview, { generator: "generator", end: END });
